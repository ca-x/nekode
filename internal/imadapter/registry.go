...
var providerSchemas = []ProviderSchema{
	{
		...
		Description:       "Telegram Bot API channel. This mirrors Stella's bot-token/channel-id shape while persisting as a Nekode InteractionEndpoint.",
		...
		SetupHints: []string{
			...
			"Optional channel_id can route notifications to a default chat.",
		},
		Fields: []Field{
	...
	},
	{
		...
		DisplayName:       "Feishu",
		Transport:         "websocket",
		Description:       "Feishu/Lark bot channel with Stella-style app credentials, group mode, and tenant hints.",
		...
		SetupHints: []string{
			...
			"Use callback URL from Nekode when webhook mode is enabled; WebSocket mode can reuse app_id/app_secret.",
		},
		Fields: []Field{
			...
			{Name: "encrypt_key", Label: "Encrypt key", Type: FieldString, Sensitive: true, Description: "Optional Feishu event encrypt key."},
			{Name: "verification_token", Label: "Verification token", Type: FieldString, Sensitive: true, Description: "Optional Feishu event verification token."},
			groupModeField(),
